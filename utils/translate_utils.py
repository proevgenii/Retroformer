...
def translate_batch_original(model, batch, eos_idx=3, beam_size=10, max_length=200,
    ...
    beam_indices = torch.arange(beam_size).to(src.device)
...
def translate_batch_accelerate(model, batch, eos_idx=3, beam_size=10, max_length=200,
                             invalid_token_indices=[], dataset=None):
    """
    :param inputs: tuple of (src, src_am, src_seg, tgt), tgt is only used to retrieve conditional reaction class token
    :param fixed_z: latent variable flag
    ...
    for step in range(0, max_length):
        ...
        # is_finished = pred_tokens[:, :, step] == eos_idx  # (batch_size, beam_size)
        # prev_scores = pred_scores.clone()
        # prev_scores[is_finished] = -1e20

        # beam_scores = unbottle_scores + prev_scores.transpose(0, 1).unsqueeze(2).expand_as(unbottle_scores)
        #
        # print(beam_scores.shape)
        # if step == 0:
        #     flat_beam_scores = beam_scores[0].view(batch_size, -1)
        # else:
        #     flat_beam_scores = beam_scores.view(batch_size, -1)
        #
        # best_scores, best_scores_id = flat_beam_scores.topk(beam_size, -1, True, True)  # best_scores: (batch_size, beam_size)
        #
        # for batch_i in range(len(batch_size)):
        #     is_finished_batch = is_finished[batch_i]
        #     best_batch_score = best_scores[batch_i]
        #
        #     best_scores_id[batch_i][:is_finished[batch_i].sum()]
        #
