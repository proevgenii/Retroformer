...
def extract_relative_mapping(cano_prod_am, cano_reacts_am):
    ...
    for i, token in enumerate(smi_tokenizer(cano_reacts_am)):
        if token[0] == '[' and token[-1] == ']' and re.match('.*:([0-9]+)]', token):
            ...
            if prod_posIdx != -1 and (not stack or stack[-1][1] == prod_posIdx - 1):
                stack.append((i, prod_posIdx))
            elif stack and prod_posIdx != -1:
                position_mapping_list.append(stack)
                stack = [(i, prod_posIdx)]
        elif stack and token in prodToken2posIdx and stack[-1][1] + 1 in prodToken2posIdx[token]:
    ...
    # Try to expand each position mapping
    new_position_mapping_list = []
    visited_react_idx, visited_prod_idx = set(), set()
    for pm_list in position_mapping_list:
        visited_react_idx.update([pm[0] for pm in pm_list])
        visited_prod_idx.update([pm[1] for pm in pm_list])
        head, tail = pm_list[0], pm_list[-1]
        if head[0] > 0 and head[1] > 0:
            if not (head[0] - 1 in visited_react_idx or head[1] - 1 in visited_prod_idx):
                if smi_tokenizer(cano_reacts_am)[head[0] - 1] == smi_tokenizer(cano_prod_am)[head[1] - 1]:
                    pm_list = [(head[0] - 1, head[1] - 1)] + pm_list
        if tail[0] < len(smi_tokenizer(cano_reacts_am)) - 1 and tail[1] < len(smi_tokenizer(cano_prod_am)) - 1:
            if not (tail[0] + 1 in visited_react_idx or tail[1] + 1 in visited_prod_idx):
                if smi_tokenizer(cano_reacts_am)[tail[0] + 1] == smi_tokenizer(cano_prod_am)[tail[1] + 1]:
                    pm_list = pm_list + [(tail[0] + 1, tail[1] + 1)]
        new_position_mapping_list.append(pm_list)
    position_mapping_list = new_position_mapping_list
