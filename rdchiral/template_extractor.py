...
def get_special_groups(mol):
    ...
    for (add_if_match, template) in group_templates:
        ...
        for match in matches:
            # sorted_idx = np.argsort(np.array(match))
            # idx2sorted_idx = {i: sorted_idx[i] for i in range(len(sorted_idx))}
            # match = sorted(match)
            add_if = []
            for pattern_idx, atom_idx in enumerate(match):
                #if idx2sorted_idx[pattern_idx] in add_if_match:
...
def get_strict_smarts_for_atom(atom):
    ...
    if 'H' not in symbol:# and atom.GetTotalNumHs() != 0: # 如果H0则不加
        ...
        num_H = re.search('H([0-9]+)', symbol)
        if num_H is None:
            H_symbol = 'H1'
            symbol = symbol.replace('H', '')
        else:
            H_symbol = 'H{}'.format(num_H.group(1))
            symbol = symbol.replace(H_symbol, '')
        if ':' in symbol: # stick H0 before label
            symbol = symbol.replace(':', ';{}:'.format(H_symbol))
        else:
            symbol = symbol.replace(']', ';{}]'.format(H_symbol))
    ...
    if '+' not in symbol and '-' not in symbol:
        ...
        if ':' in symbol:
            ...
            symbol = symbol.replace(']', ';{}]'.format(charge_symbol))
    # else:
    #     charge_flag = re.search('[+|-]', symbol).group()
    #     charge = atom.GetFormalCharge()
    #     charge_symbol = charge_flag + '{}'.format(abs(charge))
    #     symbol = symbol.replace('{}'.format(charge_flag), '')
    #     if ':' in symbol:
    #         symbol = symbol.replace(':', ';{}:'.format(charge_symbol))
    #     else:
    #         symbol = symbol.replace(']', ';{}]'.format(charge_symbol))
...
def get_fragments_for_changed_atoms(mols, changed_atom_tags, radius=0, 
    ...
    for mol in mols:
            ...
            if expansion:
                for atom in mol.GetAtoms():
                    ...
                    if label in expansion and label not in changed_atom_tags:
                        atoms_to_use.append(atom.GetIdx())
                        # Make the expansion a wildcard, WHY?
                        # Make the expansion a strictcard
                        #symbol_replacements.append((atom.GetIdx(), convert_atom_to_wildcard(atom)))
                        symbol_replacements.append((atom.GetIdx(), atom.GetSmarts()))
        ...
        while not tetra_consistent and num_tetra_flips < 100:
            ...
            #     print(sorted(atoms_to_use))
            #     print(symbols)
            #     #[x.ClearProp('molAtomMapNumber') for x in mol_copy.GetAtoms()]
            #     print('IN :', AllChem.MolFragmentToSmiles(mol_copy, atoms_to_use,
            #         atomSymbols=symbols, allHsExplicit=True,
            #         isomericSmiles=USE_STEREOCHEMISTRY, allBondsExplicit=True))
            #     print()
...
def extract_from_reaction(reaction, radius = 1):
